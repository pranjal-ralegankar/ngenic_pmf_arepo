...
/*This part below should be commented out if I want to check magnetic field output*/
...
if(0)
{
  ...
  for(i = 0, pc = 0; i < header.npart[0]; i++)
    {
      for(k = 0; k < 3; k++)
    	block[3 * pc + k] = P[i+NumPart].Vel[k]/pow(2.15*pow(10,-10),0.5)*pow(10,-9); /*I add Numpart+i because after Numpart particles we start iterating over neutrinos where I store magnetic fields */
/*I store magnetic fields in units of G and these are comoving magnetic fields; first factor converts aflven velocity to B fields in nG, second one converts to Gause*/
      pc++;
